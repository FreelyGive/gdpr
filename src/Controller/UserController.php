...
class UserController extends ControllerBase {
  ...
  public function accessCollectedData(UserInterface $user) {
    ...
    // If access to the page is enabled,
    // only show the tab if we're viewing our OWN profile
    // or we're a GDPR admin.
    if ((int) $user->id() === (int) $this->currentUser()->id()) {
      return AccessResult::allowed();
    }
    elseif ($this->currentUser()->hasPermission('view task entities')) {
      return AccessResult::allowed();
    }
